...
; 改造。基のマップに戻るためのジャンプ先
[eval exp="f.pghr = 'test_cameraworld.ks'"]
; 登録していないマップを選んだ場合の分岐用。初期値。
[eval exp="tf.isMap = false"]

; onEventにあるものと内容を合わせる
; [move]が起動していればこれはいらないが、それ以前の場合の処理。
...
; [eval exp="f.mpnm = f.NEXTMAP[tf.move.id]"]
; 2つの文字と2つのボタン
[ptext text="&'現在地:'+tf.scName" size="40"  layer="2" x="86" y="64" ]
[glink text="タイトルに戻る" target="*gotitle" x="960" y="62.5" color="btn_01_white"]
[glink text="[X]閉じる" target="*return" x="960" y="205" color="btn_27_purple" ]
[ptext text="マップワープ" color="black"  size="60" width="500"  layer="2" x="&1280/2-200" y="200" ]
...
[button graphic="&tf.wpnm" width="&1280*0.4" height="&720*0.4" x="384" y="&62.5+720*0.4+19"  target="*mpslct" exp="tf.wpnm=preexp" preexp="tf.wpnm" ]

@eval exp="tf.wpnm='map201.jpg'"

[button graphic="&tf.wpnm" width="&1280*0.4" height="&720*0.4" x="310" y="62.5"  target="*mpslct" exp="tf.wpnm=preexp" preexp="tf.wpnm" ]
...
[jump storage="&f.pghr" target="*MAPLOAD" ]
...
*warp
;そのマップにワープ。
[dialog type="confirm" storage_cancel="test_menuREF.ks"  text="&tf.wmn+'に移動します。よろしいですか?'" ]
[cm ]

; ここらへんのジャンプ情報が古いので最新に変更してそのまま使いまわす

    [chara_hide_all time="50" ]

    ; 以下部分を書き換える。tf.wmnがマップ名なのでそれをkeyにして情報を更新する。

    [iscript ]

        const moveMap = {
           // 'f201_46_01_ent': 'マップ移動前室toトリミング室',
            'f201_39_13_ant': 'マップ移動玄関2to前室',
            'f201_29_01_trm':'マップ移動前室toトリミング室',
            'f201_01_01_mac':'トリミング室toカット室',
            'f201_06_15_wgh':'カット室to計量室',
            'f101_20_01_set':'計量室toセット準備室',
            'f101_12_25_wap':'セット準備室toセット室',
            'f101_34_01_pic':'セット室to生産管理室',
            'f101_62_01_ent':'生産管理室to玄関1',
        };

        if (moveMap[tf.wmn]) {
            tf.move = tf.move || {}; // tf.moveが未定義なら初期化
            tf.move.id = moveMap[tf.wmn];

            tf.isMap = true;
        } else {
            // そのほかの場合の処理
            //alert('その部屋は登録されていません。別の部屋を選択してください。');
            tf.isMap = false;
        }

    [endscript ]

    [dialogS text="その部屋は登録されていません。別の部屋を選択してください。" cond="!tf.isMap"]
    [wait time="500" cond="!tf.isMap"]
    [jump cond="!tf.isMap"]
